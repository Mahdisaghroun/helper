...
class DaznDe extends Component {
  ...
  login = async () => {
    ...
    var data = JSON.stringify({
      Email: this.props.daznLogin.de.email,
      Password: this.props.daznLogin.de.password,
      Platform: "ios",
    });

    var config = {
      method: "post",
      url: this.props.nodeLinks.dazn_de?.key+"/loginDazn",
      headers: {
        Accept: "*/*",
  ...
  async getById(id) {
    ...
    var config = {
      ...
      url:
        ...
        format +
        "&LanguageCode=en&Model=BROWSER&Secure=true&Latitude=null&Longitude=null&Manufacturer=unknown&PlayReadyInitiator=false&MtaLanguageCode=de&AppVersion=9.20.0",
      headers: {
    ...
    let configTopass = {
      method: 'post',
      url: this.props.nodeLinks.dazn_de?.key+'/excute',
      headers: { 
        'Content-Type': 'application/json'
      },
  ...
  getSchedule(date, type) {
    var config = {
      ...
      url:
        ...
        date +
        "&country=de&languageCode=en&openBrowse=false&timeZoneOffset=60",
      headers: {},
    };
    let configTopass = {
      method: 'post',
