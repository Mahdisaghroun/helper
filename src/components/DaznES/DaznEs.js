...
class DaznDe extends Component {
  state = {
    feedback_msg: null,
    events: [],
    loading: true,
    ...
    dates: [],
    formatDate: null,
    token: localStorage.getItem("accessTokenDe"),
  ...
  login = async () => {
    ...
    var data = JSON.stringify({
      Email: this.props.daznLogin.es.email,
      Password: this.props.daznLogin.es.password,
      Platform: "ios",
    });

    var config = {
      method: "post",
      url: this.props.nodeLinks.dazn_es?.key+"/loginDazn",
      headers: {
        Accept: "*/*",
  ...
  async getById(id) {
    let format = this.state.tabId === "LIVE_KEY" ? "MPEG-DASH" : "M3U";
    var config = {
      method: "get",
      url:
        ...
        format +
        "&LanguageCode=en&Model=BROWSER&Secure=true&Latitude=null&Longitude=null&Manufacturer=unknown&PlayReadyInitiator=false&MtaLanguageCode=es&AppVersion=9.20.0",
      headers: {
        authority: "api.playback.indazn.com",
        accept: "*/*",
        "accept-language": "fr-FR,fr;q=0.9,en-US;q=0.8,en;q=0.7",
        authorization: "Bearer " + this.state.token,
       
        "x-age-verification-pin":this.props.daznLogin.de.pin??"",
    

        "x-dazn-device": "00000000-0000-0000-0000-000000000000",
      },
    };

    let configTopass = {
      method: 'post',
      url: this.props.nodeLinks.dazn_es?.key+'/excute',
      headers: { 
        'Content-Type': 'application/json'
      },
  ...
  async componentDidUpdate(previousProps, previousState) {
    if (previousState.formatDate !== this.state.formatDate||previousState.token !== this.state.token) {
      ...
      this.setState({ events: res.data.Tiles });
      this.setState({ temp: res.data.Tiles });
  ...
  getSchedule(date, type) {
    var config = {
      ...
      url:
        ...
        date +
        "&country=es&languageCode=en&openBrowse=false&timeZoneOffset=60",
      headers: {},
    };
    let configTopass = {
      method: 'post',
  ...
  async componentDidMount() {
    try {
      ...
      this.setState({ ipInfo: ipres.data });
      this.setState({ loading: true });
      const format2 = "YYYY-MM-DD";
    ...
    return (
      <SidebarTemplate>
        ...
        <div className="text-center mt-3">
          ...
          {this.props?.isAdmin&&     <button className="btn btn-warning float-right" onClick={()=>this.setState({loginModalState:true})}>NEW LOGIN</button>}
        </div>
        <div
...
>
    ...
    onChange={(e)=>this.onSelect(e.value)}
      ...
      />
        ...
        {this.state.tabId == "SCHEDULE" && (
          <div
          // className="row br schedule__header-wrapper"
          >
            <div
            // className="col-md-11"
            >
       ...
       {this.state.loginModalState&& <ModalLogin reload={(token)=>this.setState({token:token})} onClose={()=>this.setState({loginModalState:false})}></ModalLogin>}
      </SidebarTemplate>
    );
  }
}
