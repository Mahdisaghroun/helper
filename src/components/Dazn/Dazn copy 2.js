...
class Dazn extends Component {
  ...
  async getById(id) {
    ...
    let configg = {
      ...
      headers: {
        ...
        "x-age-verification-pin":this.props.daznLogin.it.pin??"",
  ...
  login = async () => {
    ...
    var config = {
      method: "post",
      url: this.props.nodeLinks.dazn_it?.key+"/loginDazn",
      headers: {
        Accept: "*/*",
        ...
        try {
          ...
          await Promise.all(
            channels.map(async (el, i) => {
              try {
                let response = await this.getById(el.AssetId);
            //    console.log("xml",response.data.PlaybackDetails)
                Axios.get(response.data.PlaybackDetails[0].ManifestUrl).then(res=>{
                 console.log("xml2",res.data)
                })
                channels[i].urls = response.data.PlaybackDetails;
              } catch (error) {
                console.log(error);
      ...
      await Promise.all(
        channels.map(async (el, i) => {
          try {
            ...
            console.log("hahahahahahahah",d1)
            fetch(d1)
            .then(response => response.text())
            .then(str => (new window.DOMParser()).parseFromString(str, "text/xml"))
            .then(data => {
            // console.log('fuck2',data)
              const cubes = data.getElementsByTagName("cenc:pssh");
              let pssh = cubes[0].innerHTML
             // console.log('fuck',"AAAAXHBzc2gAAAAA7e+LqXnWSs6jyCfc1R0h7QAAADwIARIQvbMxXFLnWfKIwNV1U/dsQhoIdXNwLWNlbmMiGHZiTXhYRkxuV2ZLSXdOVjFVL2RzUWc9PSoAMgA".length)
              channels[i].pssh=pssh
